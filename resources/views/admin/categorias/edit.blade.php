...
@section('content')
    <h1 class="page-title">MODIFICAR CATEGORÍA</h1>
    <p class="page-subtitle">Actualiza los datos de la categoría</p>
        ...
        <form method="POST" action="{{ route('admin.categorias.update', $categoria) }}">
            ...
            <div class="form-grid">
                <div class="form-group full-width">
                    ...
                    <input id="nombre" type="text" name="nombre" value="{{ old('nombre', $categoria->nombre) }}" class="form-control" required autofocus>
                    @error('nombre')<p class="form-error">{{ $message }}</p>@enderror
                </div>

                <div class="form-group full-width">
                    <label for="descripcion">Descripción</label>
                    <textarea id="descripcion" name="descripcion" class="form-control" required>{{ old('descripcion', $categoria->descripcion) }}</textarea>
                    @error('descripcion')<p class="form-error">{{ $message }}</p>@enderror
                </div>

                <div class="form-group full-width">
                    <label for="imagen">Imagen (URL)</label>
                    <input id="imagen" type="text" name="imagen" value="{{ old('imagen', $categoria->imagen) }}" class="form-control" placeholder="https://ejemplo.com/imagen.jpg">
                ...
                <div class="form-group">
                    ...
                    <select id="categoria_padre_id" name="categoria_padre_id" class="form-control">
                        ...
                        @foreach ($categorias as $padre)
                            <option value="{{ $padre->id }}" {{ old('categoria_padre_id', $categoria->categoria_padre_id) == $padre->id ? 'selected' : '' }}>{{ $padre->nombre }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <div class="checkbox-group">
                        <input id="solo_local" type="checkbox" name="solo_local" value="1" {{ old('solo_local', $categoria->solo_local) ? 'checked' : '' }}>
                        <label for="solo_local">Solo local</label>
                    </div>
                    <div class="checkbox-group">
                        <input id="solo_tienda" type="checkbox" name="solo_tienda" value="1" {{ old('solo_tienda', $categoria->solo_tienda) ? 'checked' : '' }}>
